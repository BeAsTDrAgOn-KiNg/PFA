...
const AdoptionPage: React.FC = () => {
  const [view, setView] = useState<'register' | 'summary'>('summary'); 
  ...
  const FormContent = ({ data, isPreview = false }: { data: any, isPreview?: boolean }) => (
    <div className="relative bg-white min-h-[1100px] shadow-2xl rounded-sm p-8 md:p-20 overflow-hidden text-left">
      ...
      <div className="space-y-8">
        <UnderlineInput disabled={isPreview} label="Your Name" value={data.adopterName} onChange={(e: any) => setFormData({...formData, adopterName: e.target.value})} placeholder="Full Name" />
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          <UnderlineInput disabled={isPreview} label="Age" value={data.adopterAge} onChange={(e: any) => setFormData({...formData, adopterAge: e.target.value})} placeholder="Years" />
          <div className="flex items-center gap-4 text-[13px] font-bold text-slate-800 uppercase tracking-tight">
            GENDER : 
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input disabled={isPreview} type="radio" checked={data.adopterGender === 'Male'} onChange={() => setFormData({...formData, adopterGender: 'Male'})} className="w-4 h-4 border-slate-300 accent-[#005F54]" /> 
              <span>Male</span>
            </label>
            <span>/</span>
            <label className="flex items-center gap-1.5 cursor-pointer">
              <input disabled={isPreview} type="radio" checked={data.adopterGender === 'Female'} onChange={() => setFormData({...formData, adopterGender: 'Female'})} className="w-4 h-4 border-slate-300 accent-[#005F54]" /> 
              <span>Female</span>
            </label>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          <UnderlineInput disabled={isPreview} label="Mobile No" value={data.phone} onChange={(e: any) => setFormData({...formData, phone: e.target.value})} placeholder="91..." />
          <UnderlineInput disabled={isPreview} label="Email" value={data.email} onChange={(e: any) => setFormData({...formData, email: e.target.value})} placeholder="email@address.com" />
        </div>

        <UnderlineInput disabled={isPreview} label="Home Address" value={data.address} onChange={(e: any) => setFormData({...formData, address: e.target.value})} placeholder="Street, Colony, Landmark" />
        ...
        <div className="pt-6 space-y-4">
          ...
          <input disabled={isPreview} className="w-full bg-transparent border-b border-slate-400 focus:outline-none py-1 text-[#1e40af] font-bold italic" value={data.animalType} onChange={e => setFormData({...formData, animalType: e.target.value})} placeholder="Animal type" />
        </div>

        <div className="flex items-center gap-4 text-[13px] font-bold text-slate-800 uppercase tracking-tight">
          GENDER OF ANIMAL : 
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input disabled={isPreview} type="radio" checked={data.targetGender === 'Male'} onChange={() => setFormData({...formData, targetGender: 'Male'})} className="w-4 h-4 border-slate-300 accent-[#005F54]" /> 
            <span>Male</span>
          </label>
          <span>/</span>
          <label className="flex items-center gap-1.5 cursor-pointer">
            <input disabled={isPreview} type="radio" checked={data.targetGender === 'Female'} onChange={() => setFormData({...formData, targetGender: 'Female'})} className="w-4 h-4 border-slate-300 accent-[#005F54]" /> 
            <span>Female</span>
          </label>
        </div>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center gap-2">
          ...
          <input disabled={isPreview} className="flex-1 bg-transparent border-b border-slate-400 text-[#1e40af] font-bold w-full md:w-auto mt-2 md:mt-0" value={data.targetColor} onChange={e => setFormData({...formData, targetColor: e.target.value})} placeholder="Color pattern" />
        </div>

        <UnderlineInput disabled={isPreview} label="Why do you want to adopt?" value={data.reason} onChange={(e: any) => setFormData({...formData, reason: e.target.value})} placeholder="Reason" />
          ...
          <div className="relative w-full max-w-5xl animate-in slide-in-from-bottom-8 duration-300 mb-20 space-y-12">
            <div className="flex justify-between items-center bg-white/20 backdrop-blur-md p-6 rounded-3xl border border-white/20 shadow-2xl">
               <div className="flex items-center gap-4">
                 ...
                 <div>
                   <h2 className="text-white text-xl font-black uppercase tracking-widest">Form View</h2>
               ...
               <div className="flex gap-4">
                 ...
                 <button onClick={() => setViewingRecord(null)} className="p-3 bg-rose-500 text-white rounded-2xl shadow-xl hover:bg-rose-600 transition-all"><X size={24} /></button>
               </div>
            </div>
            <div className="space-y-12 pb-20">
               <FormContent data={viewingRecord} isPreview={true} />
               <DeclarationContent data={viewingRecord} isPreview={true} />
            </div>
          </div>
        </div>
      )}
